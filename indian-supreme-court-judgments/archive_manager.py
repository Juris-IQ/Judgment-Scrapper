...
class S3ArchiveManager:
    ...
    def _load_index_from_s3(self, year: int, archive_type: str) -> IndexFileV2:
        ...
        if self.local_only:
            return self._load_index_from_local_archives(year, archive_type)
    ...
    def _load_index_from_local_archives(
        self, year: int, archive_type: str
    ) -> IndexFileV2:
        """Build an index from existing local tar files for resume-safe retries."""
        now = ist_now_iso()
        index = IndexFileV2(
            year=year,
            archive_type=archive_type,
        ...
        archive_names = [f"{archive_type}{self._get_archive_extension(archive_type)}"]
        archive_names.extend(
            sorted(path.name for path in year_dir.glob("part-*.tar") if path.is_file())
        )
        ...
        for archive_name in archive_names:
            ...
            try:
                ...
                logger.warning("Could not index local archive %s: %s", archive_path, e)
                continue
            ...
            part = IndexPart(
                ...
                size_human=format_size(archive_path.stat().st_size),
                created_at=now,
            )
            index.add_part(part)
            ...
            if local_path and local_path.exists():
                ...
                try:
                    ...
                    if not self.local_only:
                        raise
                    logger.warning(
                        "Could not open local archive %s for append: %s. "
                        "Rewriting archive before retrying append.",
                        local_path,
                        e,
                    )
                    self._rewrite_local_archive(local_path)
                    archive = tarfile.open(local_path, "a")
    ...
    def _rewrite_local_archive(self, archive_path: Path) -> None:
        """Rewrite a readable tar archive so Python can append to it reliably."""
        ...
        try:
            with tarfile.open(archive_path, "r") as source:
                ...
                with tarfile.open(temp_path, "w") as target:
